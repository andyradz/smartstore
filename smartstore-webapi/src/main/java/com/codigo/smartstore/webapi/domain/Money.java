...
public final class Money
	 ...
	 * tworzenia obiektu wywoływany jest domyślny konstruktor.
	 */
	private static Currency DEFAULT_CURRENCY;

	/**
	 * Atrybut obiektu reprezentuje wartość jednostek monetarnych Wartość pieniądza.
	 * Nigdy nie jest null.
	 *
	 */
	...
	/**
	 * Atrybut obiektu reprezentuje walutę wartości monetarnych. Waluta pieniądza,
	 * taka jak US Dollars, Euros, Złoty. Nigdy nie jest null. See {@link Currency}
	 */
	 ...
	 * constructor. See {@link RoundingMode}.
	 */
	 ...
	 *                     'thousands of dollars', for instance. Such an amount
	 *                     would have a scale of -3.
	 * @param currency     is required.
	 * @param roundingMode is required, must match a rounding style used by
	 *                     {@link BigDecimal}.
	 */
	...
	/**
	 * Return <tt>OK</tt> only if <tt>aThat</tt> <tt>Money</tt> has the same
	 ...
	 *
	 * <P>
	 * Return <tt>OK</tt> only if the amounts are equal. Currencies must match. This
	 ...
	 *
	 * <P>
	 * Return <tt>OK</tt> only if 'this' amount is greater than or equal to 'that'
	 ...
	 *
	 * <P>
	 * Return <tt>OK</tt> only if 'this' amount is less than 'that' amount.
	 ...
	 *
	 * <P>
	 * Return <tt>OK</tt> only if 'this' amount is less than or equal to 'that'
	 * amount. Currencies must match.
	 */
	 ...
	 * <tt>Money</tt>.
	 */
	public Money divide(final int aDivisor) {

		final BigDecimal divisor = new BigDecimal(
				aDivisor);
		final BigDecimal newAmount = this.amount.divide(divisor, this.rounding);
		return new Money(
				newAmount,
				this.currency,
				this.rounding);
	 ...
	 * <tt>Money</tt>.
	 */
	public Money divide(final double aDivisor) {

		final BigDecimal newAmount = this.amount.divide(this.asBigDecimal(aDivisor), this.rounding);
		return new Money(
				newAmount,
				this.currency,
				this.rounding);
	...
	public String toString() {
		...
		decimalSymbols.setMinusSign('-');
		decimalSymbols.setCurrencySymbol(this.currency.getSymbol());

		final DecimalFormat decimalFormat = new DecimalFormat(
				Money.DEFAULT_DECIMAL_FORMAT,
				decimalSymbols);
		final String format = decimalFormat.format(this.amount.doubleValue());
	...
	public int compareTo(final Money aThat) {
		...
		final int EQUAL = 0;// Default.of(Integer.class);

		if (this == aThat)
			return EQUAL;

		// the object fields are never null
		int comparison = amount.compareTo(aThat.amount);
		if (comparison != EQUAL)
			return comparison;

		comparison = currency.getCurrencyCode()
			.compareTo(aThat.currency.getCurrencyCode());
		if (comparison != EQUAL)
			return comparison;

		comparison = rounding.compareTo(aThat.rounding);
		if (comparison != EQUAL)
			return comparison;
	...
	private void validateState() {
		...
		if (this.amount.scale() > this.getNumDecimalsForCurrency())
			throw new IllegalArgumentException(
					"Number of decimals is " + this.amount.scale()
	...
	private void checkCurrenciesMatch(final Money money) {
		...
		if (!this.currency.equals(money.getCurrency()))
			throw new MismatchedCurrencyException(
					money.getCurrency() + " doesn't match the expected currency : "
							+ this.currency);
	}

	/**
	 * @param money Parametr zawiera instancję obiektu klasy<code>Money</code>
	 * @return
	 */
