...
public class TimeSpan
	 ...
	 * second. There are 10,000 ticks in a millisecond. A tick represents the total
	 * number of ticks in local time, which is midnight on January 1st in the year
	 * 0001. But a tick is also smallest unit for TimeSpan also. Since ticks are
	 * Int64, so if miliseconds used instead of ticks, there can be a information
	 * losing.
	 */
	...
	// private static final DoubleUnaryOperator evalueTotalMilliseconds = value -> {
	//
	// double num = value;
	...
	public TimeSpan() {
		...
		this.ticks = Valuable.none();
		this.days = Valuable.none();
		this.hours = Valuable.none();
		this.minutes = Valuable.none();
		this.seconds = Valuable.none();
		this.milliseconds = Valuable.none();
		this.totalDays = Valuable.none();
		this.totalHours = Valuable.none();
		this.totalMinutes = Valuable.none();
		this.totalSeconds = Valuable.none();
		this.totalMilliseconds = Valuable.none();
		// this.totalMilliseconds = Valuable.from(null, UnaryOperator.identity(),
		// TimeSpan.evalueTotalMilliseconds);
	}
	...
	public TimeSpan(final int days, final int hours, final int minutes, final int seconds, final int milliseconds) {
		...
		this.ticks.set(
			(days * TimeSpan.TICKSPERDAY) + (hours * TimeSpan.TICKSPERHOUR)
	...
	public int compareTo(final TimeSpan timespan) {
		...
		final Comparator<TimeSpan> comparator = (
				final TimeSpan a, final TimeSpan b
		) -> {

			if (Objects.isNull(a))
				return (Objects.isNull(b)) ? CompareResult.EQUAL.get() : CompareResult.LESS.get();

			if (Objects.isNull(b))
				return CompareResult.MORE.get();

			if (b == a)
				return CompareResult.EQUAL.get();
