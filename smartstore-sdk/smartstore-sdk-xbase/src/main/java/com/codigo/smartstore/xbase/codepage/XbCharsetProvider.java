...
import com.codigo.smartstore.sdk.core.opearte.EnumOperator;

/**
 * Klasa realizuje mechanizm udostępniania nowych mechanizmów kodowania znaków.
 *
 * Nowe standardy można wykorzystać w środowisku jre w standardowy sposób
 * wskazywania kodowania znaków z Charset.
 *
 ...
 * @category charset
 */
public final class XbCharsetProvider
		extends
		CharsetProvider {

	/**
	 * Typ wyliczeniowy do określenia standarów kodowania znaków dla danych
	 * zapisanych w formacie XBase
	 */
	private enum XbCharsets {

		/**
		 * ISO 8859-2 lub bardziej formalnie ISO/IEC 8859-2, również znane jako ISO
		 * Latin-2, bądź "środkowo–" i "wschodnioeuropejskie", jest drugą częścią
		 * standardu kodowania znaków zdefiniowanego przez organizację ISO. Składa się
		 * ze 191 znaków łacińskiego pisma, z czego każdy jest zapisywany przy pomocy
		 * ośmiu bitów. Na jego podstawie została utworzona Polska Norma
		 * (PN-T-42118:1993) opisująca kodowanie polskich liter diakrytyzowanych w
		 * kodach 8 bitowych.
		 */
		...
		/**
		 * ROT47 – kodowanie przesuwające, zamieniające każdy znak ASCII z przedziału
		 * 33-126 na znak znajdujący się 47 pozycji dalej, ale nie dalej niż do 126
		 * pozycji. Podobnie jak ROT13 jest on samoodwracalny.
		 */
		ROT47("ROT47"),
		 ...
		 * polskiego komputera klasy IBM PC o nazwie handlowej Mazovia. Z inicjatywy
		 * Andrzeja Gecowa otrzymało ono od 1.IX.1992 status Normy Zakładowej ZN-92
		...
		/**
		 * kodowanie 8-bitowe znaków przeznaczone do stosowania w systemach operacyjnych
		 * MS-DOS i kompatybilnych. Zostało stworzone przez braci Kamenickich z
		 * Czechosłowacji, by umożliwić stosowanie czeskich i słowackich liter na
		 * komputerach osobistych. Mimo wejścia na rynek systemu operacyjnego MS-DOS w
		 * wersji 5.0, posiadającego natywną obsługę języków Europy Środkowej (w postaci
		 * strony kodowej CP852), a następnie Windows 3.x (używającego strony kodowej
		 * CP1250), kodowanie Kamenický dalej było powszechnie stosowane. Dopiero od
		 * czasu rozpowszechnienia się systemów operacyjnych Windows 95 i Windows NT
		 * 4.0, wykorzystujących kodowanie Windows-1250 i (w różnym stopniu) unikod,
		 * znaczenie tego kodowania zaczęło bardzo szybko maleć, choć wciąż można
		 ...
		 *
		 * @param charsetName Nazwa standardu kodowania znaków
		 * @param checksetAliases Aliasy nazw standardu kodowania znaków
		 */
	...
	public Charset charsetForName(final String charsetName) {
		...
		if (EnumOperator.getName(XbCharsets.class, charsetName.trim())
